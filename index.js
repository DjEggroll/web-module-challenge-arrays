...
const copiedArray = copy(originalFlavors);
...
function removeFlavorByName(currentFlavors, flavorName){
  currentFlavors.splice(currentFlavors.indexOf(flavorName), 1);
  return currentFlavors
  /*your code here*/
}
...
function filterByWord(currentFlavors, filter){
  let filteredFlavors = [];
  for(let i = 0; i < currentFlavors.length; i++){
    if(currentFlavors[i].toUpperCase().includes(filter.toUpperCase())){
      filteredFlavors.push(currentFlavors[i]);
    }
  }
...
function getAverageWordLength(currentFlavors){
  let sum = 0;
  for(let i = 0; i < currentFlavors.length; i++){
    sum = sum + currentFlavors[i].split(' ').length;
    }
  let average = (sum / currentFlavors.length);
  return average;
  /*code here*/
}
...
function getRandomFlavors(originalF, newF, seasonalF, regionalF){
  let allFlavors = originalF.concat(newF, seasonalF, regionalF);
  let randomFlavors = [];
  for(let i = 0; i < 31; i++){
      let randomChoice = Math.floor((Math.random() * allFlavors.length));
      randomFlavors.push(allFlavors[randomChoice]);
      allFlavors.splice(randomChoice, 1);
    }
